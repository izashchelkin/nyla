...
$scriptBlock = {
    $privateCommon = $files | Where-Object { $_.Extension -eq '.cc' -and $_.BaseName -notmatch '(_windows|_linux|_vulkan)$' }
    $publicCommon = $files | Where-Object { $_.Extension -eq '.h' -and $_.BaseName -notmatch '(_windows|_linux|_vulkan)$' }
