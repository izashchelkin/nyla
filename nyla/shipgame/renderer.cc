...
//     VK_CHECK(
//         vkCreateDescriptorSetLayout(vk.device, &descriptor_set_layout_create_info, nullptr, &descriptor_set_layout));
...
//     };
//
//     VK_CHECK(vkAllocateDescriptorSets(vk.device, &descriptor_set_alloc_info, per_frame->descriptor_sets.data()));
//   }
//
//   for (size_t i = 0; i < kVulkan_NumFramesInFlight; ++i) {
//     CreateMappedBuffer(per_frame->descriptor_sets[i], 0, false, sizeof(SceneUbo), sizeof(SceneUbo),
//                        per_frame->scene_ubo.buffer[i], per_frame->scene_ubo.memory[i], per_frame->scene_ubo.mapped[i]);
//
//     CreateMappedBuffer(per_frame->descriptor_sets[i], 1, true, (256 * sizeof(GameObjectUbo)), sizeof(GameObjectUbo),
//                        per_frame->gameobject_ubo.buffer[i], per_frame->gameobject_ubo.memory[i],
...
//   };
//
//   memcpy(per_frame->scene_ubo.mapped[vk.current_frame_data.iframe], &scene_ubo, sizeof(scene_ubo));
// }
//
...
//   memcpy(per_frame->gameobject_ubo.mapped[vk.current_frame_data.iframe], &renderq.ubo,
//          sizeof(renderq.ubo[0]) * std::size(renderq.ubo));
// }
//
// void RenderGameObjectecord() {
...
//                             &per_frame->descriptor_sets[vk.current_frame_data.iframe], 1, &ubo_offset);
//
