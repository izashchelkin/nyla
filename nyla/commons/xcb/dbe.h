...
#define XCB_DBE_GET_BACK_BUFFER_ATTRIBUTES 7
     ...
     * @param major_version The major version of the extension. Check that it is compatible with the
     * XCB_DBE_MAJOR_VERSION that your code is compiled with.
     * @param minor_version The minor version of the extension. Check that it is compatible with the
     * XCB_DBE_MINOR_VERSION that your code is compiled with.
     * @return A cookie
     *
     * Queries the version of this extension. You must do this before using any functionality it provides.
     *
     */
    xcb_dbe_query_version_cookie_t xcb_dbe_query_version(xcb_connection_t *c, uint8_t major_version,
                                                         uint8_t minor_version);
     ...
     */
    xcb_dbe_query_version_cookie_t xcb_dbe_query_version_unchecked(xcb_connection_t *c, uint8_t major_version,
                                                                   uint8_t minor_version);
     ...
     * @param swap_action The swap action most likely to be used to present this back buffer. This is only a hint, and
     * does not preclude the use of other swap actions.
     * @return A cookie
     *
     * Associates \a buffer with the back buffer of \a window. Multiple ids may be associated with the back buffer,
     * which is created by the first allocate call and destroyed by the last deallocate.
     *
     * This form can be used only if the request will not cause
     * a reply to be generated. Any returned error will be
     * saved for handling by xcb_request_check().
     */
    xcb_void_cookie_t xcb_dbe_allocate_back_buffer_checked(xcb_connection_t *c, xcb_window_t window,
                                                           xcb_dbe_back_buffer_t buffer, uint8_t swap_action);
     ...
     *
     */
    xcb_void_cookie_t xcb_dbe_allocate_back_buffer(xcb_connection_t *c, xcb_window_t window,
                                                   xcb_dbe_back_buffer_t buffer, uint8_t swap_action);
     ...
     * @return A cookie
     *
     * Deallocates the given \a buffer. If \a buffer is an invalid id, a `BadBuffer` error is returned. Because a window
     * may have allocated multiple back buffer ids, the back buffer itself is not deleted until all these ids are
     ...
     * @return A cookie
     *
     * Swaps the front and back buffers on the specified windows. The front and back buffers retain their ids, so that
     * the window id continues to refer to the front buffer, while the back buffer id created by this extension
     * continues to refer to the back buffer. Back buffer contents is moved to the front buffer. Back buffer contents
     * after the operation depends on the given swap action. The optimal swap action depends on how each frame is
     * rendered. For example, if the buffer is cleared and fully overwritten on every frame, the "untouched" action,
     * which throws away the buffer contents, would provide the best performance. To eliminate visual artifacts, the
     * swap will occure during the monitor VSync, if the X server supports detecting it.
     *
     * This form can be used only if the request will not cause
     * a reply to be generated. Any returned error will be
     * saved for handling by xcb_request_check().
     */
    xcb_void_cookie_t xcb_dbe_swap_buffers_checked(xcb_connection_t *c, uint32_t n_actions,
     ...
     * @return A cookie
     *
     * Creates a block of operations intended to occur together. This may be needed if window presentation requires
     * changing buffers unknown to this extension, such as depth or stencil buffers.
     *
     * This form can be used only if the request will not cause
     * a reply to be generated. Any returned error will be
     * saved for handling by xcb_request_check().
     ...
     *
     */
    xcb_dbe_get_visual_info_cookie_t xcb_dbe_get_visual_info(xcb_connection_t *c, uint32_t n_drawables,
     ...
     * placed in the event queue.
     */
    xcb_dbe_get_visual_info_cookie_t xcb_dbe_get_visual_info_unchecked(xcb_connection_t *c, uint32_t n_drawables,
                                                                       const xcb_drawable_t *drawables);

    int xcb_dbe_get_visual_info_supported_visuals_length(const xcb_dbe_get_visual_info_reply_t *R);

    xcb_dbe_visual_infos_iterator_t xcb_dbe_get_visual_info_supported_visuals_iterator(
     ...
     * placed in the event queue.
     */
    xcb_dbe_get_back_buffer_attributes_cookie_t xcb_dbe_get_back_buffer_attributes_unchecked(
        xcb_connection_t *c, xcb_dbe_back_buffer_t buffer);
     ...
     * The returned value must be freed by the caller using free().
     */
    xcb_dbe_get_back_buffer_attributes_reply_t *xcb_dbe_get_back_buffer_attributes_reply(
        xcb_connection_t *c, xcb_dbe_get_back_buffer_attributes_cookie_t cookie /**< */, xcb_generic_error_t **e);
