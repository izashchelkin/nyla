...
void API WinSetTitle(byteview title)
{
    ...
    xcb_change_property(X11GetConn(), XCB_PROP_MODE_REPLACE, win, atoms.wm_name, XCB_ATOM_STRING, 8, title.size,
                        title.data);
    xcb_change_property(X11GetConn(), XCB_PROP_MODE_REPLACE, win, atoms.net_wm_name, atoms.utf8_string, 8, title.size,
                        title.data);
    X11Flush();
}
#endif // NYLA_HEADLESS
...
void API HandleXSelectionRequest(xcb_selection_request_event_t *ev)
{
    ...
    {
        xcb_change_property(X11GetConn(), XCB_PROP_MODE_REPLACE, ev->requestor, ev->property, atoms.utf8_string, 8,
                            platform->x11.clipboardContent.size, platform->x11.clipboardContent.data.data);
    }
    else if (ev->target == atoms.targets)
    {
        xcb_atom_t targets[] = {atoms.utf8_string};
        xcb_change_property(X11GetConn(), XCB_PROP_MODE_REPLACE, ev->requestor, ev->property, XCB_ATOM_ATOM, 32,
