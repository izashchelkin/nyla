...
struct DBusErrorWrapper {
  ...
  DBusErrorWrapper() { dbus_error_init(&inner); }
  ~DBusErrorWrapper() { dbus_error_free(&inner); }
  ...
  operator DBusError*() { return &inner; }
  operator const DBusError*() const { return &inner; }

  bool bad() const { return dbus_error_is_set(&inner); }
  const char* name() const { return inner.name; }
  const char* message() const { return inner.message; }
