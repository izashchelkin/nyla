...
int Main()
{
    ...
    {
        ASSERT(false && "Not PSF2");
    }
    if (header_size < 32 || buf.Size() < header_size)
    {
        ASSERT(false && "Bad/truncated header");
    }
    ...
    {
        LOG("glyph_size mismatch header=%d computed=%d (continuing)", glyph_size, expect_glyph);
    }
    const uint64_t glyph_table_bytes = uint64_t(length) * glyph_size;
    if (buf.Data() + buf.Size() < glyphs + glyph_table_bytes)
    {
        ASSERT(false && "Truncated glyph table");
    }
    ...
    {
        ASSERT(false && "This generator targets up to 8x32 fonts");
        return 2;
    }
    ...
    for (uint32_t c = 0x20; c <= 0x7F; ++c)
    {
        ...
        {
            LOG() << "Note: codepoint U+" << std::hex << std::uppercase << c << " not mapped; emitting blank glyph.\n"
                  << std::dec;
        }
